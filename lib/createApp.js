...
function createApp(opts = {}) {
  ...
  // Le versement de pièces transporte des documents (base64) : sa route reçoit un
  // plafond dédié, AVANT le parseur général — un corps déjà lu n'est pas relu.
  app.use('/api/v1/workflow/dossiers', express.json({ limit: '6mb' }));
  ...
  // --- Pièces du dossier : versement électronique et restitution -----------
  // Le document entre au guichet avec le dossier : haché, horodaté, imputé,
  // tracé — et son empreinte rejoint l'archive scellée à la clôture.
  app.post('/api/v1/workflow/dossiers/:id/pieces', requireModule('workflow'), (req, res) => {
    try {
      const b = req.body || {};
      const r = workflow.verserPiece(req.params.id, { nom: b.nom, mime: b.mime, base64: b.base64 }, req.auth);
      auditEvent(req, 'WF_PIECE_VERSEE', r.dossier.numero, { piece: r.piece.id, nom: r.piece.nom, sha256: r.piece.sha256, octets: r.piece.octets });
      res.status(201).json({ piece: r.piece, dossier: workflow.toPublicDossier(r.dossier, req.auth) });
    } catch (e) { res.status(400).json({ error: e.message }); }
  });

  app.get('/api/v1/workflow/dossiers/:id/pieces/:pieceId', requireModule('workflow'), (req, res) => {
    try {
      const { meta, contenu } = workflow.lirePiece(req.params.id, req.params.pieceId, req.auth);
      auditEvent(req, 'WF_PIECE_CONSULTEE', req.params.id, { piece: meta.id, nom: meta.nom });
      res.setHeader('Content-Type', meta.mime);
      res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(meta.nom)}"`);
      res.setHeader('X-Content-SHA256', meta.sha256);
      res.send(contenu);
    } catch (e) {
      const altere = /ALTÉRATION/.test(e.message);
      res.status(altere ? 500 : 404).json({ error: e.message });
    }
  });

  // --- Archive électronique : l'enregistrement scellé d'un dossier clos ----
  // Restitué avec la clé publique : la vérification est possible HORS plateforme.
  app.get('/api/v1/workflow/dossiers/:id/archive', requireModule('workflow'), (req, res) => {
    const d = workflow.get(req.params.id);
    if (!d || !workflow.visible(d, req.auth)) return res.status(404).json({ error: 'Dossier introuvable.' });
    const archive = archives.get(d.numero);
    if (!archive) return res.status(404).json({ error: 'Dossier non encore archivé (il n\'a pas atteint de statut terminal).' });
    auditEvent(req, 'WF_ARCHIVE_CONSULTEE', d.numero, { seq: archive.seq });
    res.json({ archive, note: 'Vérifiable hors plateforme : recalculer le hash du payload (JSON canonique) et contrôler la signature avec publicKeyPem.' });
  });

  // Intégrité du registre des archives (portée annoncée, comme les autres chaînes).
  app.get('/api/v1/workflow/archives/verify', requireModule('workflow'), (req, res) => {
    res.json(archives.verify(req.query.full === '1' ? {} : { limit: 2000 }));
  });
