...
async function authCookie(username = 'regulateur') {
...
test('RBAC : un rôle sans le module reçoit 403', async () => {
  // observatoire n'a pas accès au module admin/config.
  const r = await fetch(base + '/api/v1/config', { headers: { Cookie: await authCookie('observatoire') } });
