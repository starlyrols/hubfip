...
// souverain, accès tracé) mais expose un MSISDN masqué via redact() à l'égress.
// =============================================================================
...
function buildTDR(input = {}) {
  ...
  // Cellules émettrice / réceptrice (corrélation réseau).
  ...
  const cellDest = ref.cellById.get(input.cellDestId)
    || (interop ? ref.cellsOf((ref.cellById.get(input.cellOriginId) || cellOrigin).city)[0] : cellOrigin);
  ...
  return {
    ...
    location: { city: cellOrigin.city, province: cellOrigin.province, lat: cellOrigin.lat, lng: cellOrigin.lng },
    legs,
...
// Minimisation à l'égress : masque les MSISDN sauf si reveal=true (accès nominatif
// encadré, à journaliser par l'appelant). Renvoie une COPIE.
function redact(tdr, reveal) {
  if (!tdr) return tdr;
  const m = (msisdn) => (reveal ? msisdn : ref.maskMsisdn(msisdn));
  return {
    ...tdr,
    sender: { ...tdr.sender, msisdn: m(tdr.sender.msisdn), msisdnFull: undefined },
    receiver: { ...tdr.receiver, msisdn: m(tdr.receiver.msisdn) },
    _redacted: !reveal,
  };
}
...
function toPublic(tdr, { reveal = false } = {}) {
  ...
  return {
    ...
    agent: tdr.agent,
    cellOrigin: tdr.cellOrigin, location: tdr.location, latencyMs: tdr.latencyMs,
    source: tdr.source,
...
module.exports = {
  STATUS, FAIL_RATE_DEFAULT, buildTDR, fromIso8583, redact, toPublic, computeFee, nextStan,
  // Alias de compatibilité (anciens appels) :
  buildTransaction: buildTDR,
};
