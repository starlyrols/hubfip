...
| 12 | **Sécurité, RBAC & Audit** | RBAC par corps de métier, **journal d'audit inviolable**, minimisation des données — [`users.js`](lib/users.js), [`audit.js`](lib/audit.js) |
...
lib/{auth,users,config,audit,subjects}.js           Sessions, RBAC, config, audit, jetons de sujet
lib/createApp.js       API REST durcie (helmet/CSP, CORS, rate-limit) + RBAC + audit
public/                Front : login + espace de travail (13 onglets role-gated), charts, carte
```
...
## Rôles & cloisonnement (RBAC par corps de métier)

Une page de connexion donne accès à l'espace de chaque **intervenant**. Accès aux modules **appliqué
côté serveur** (`requireModule`), cloisonnement opérateur, et **minimisation des données** :

| Profil | Accès |
|---|---|
| **Direction / Supervision** (`regulateur`) | Tous les modules |
| **Observatoire** | Statistiques, opérateurs, géo, registre, reporting |
| **Régulation économique** (`revenus`) | Revenus & redevances, observatoire, reporting |
| **Technique / QoS** | QoS, connecteurs, géo, observatoire, reporting |
| **Antifraude / AML** | Antifraude, investigation, géo, analytics + **révélation MSISDN encadrée** |
| **Juridique / Conformité** | Investigation, antifraude, sécurité/audit, reporting |
| **Protection consommateurs** | QoS, observatoire, reporting |
| **Administrateur SI** | Administration, connecteurs, sécurité, géo |
| **Auditeur** | Vérification & exports (lecture seule) |
| **Opérateur** (`airtel`/`moov`/`gimac`) | **Restreint à ses propres flux** (REST + WebSocket filtrés) |

**Accès démo « un clic » (DEV)** : la page de connexion propose un bouton par compte (sans mot de
passe). **Désactivé** avec `NODE_ENV=production`. Mot de passe commun (connexion classique) :
`SUMO_DEMO_PASSWORD` (défaut `demo123`).
