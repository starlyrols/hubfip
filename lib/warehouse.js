...
function selectRecent({ operatorId, sinceEpoch, type, channel } = {}) {
  return buffer.filter((t) =>
    (!operatorId || t.operator.id === operatorId || t.receiverOperator.id === operatorId)
...
function snapshot(opts = {}) {
  ...
  const byOperator = by((t) => t.operator.id).map((e) => {
    const op = ref.byId.get(e.key);
    return { ...e, name: op ? op.name : e.key, engine: op ? op.engine : null, color: op ? op.color : null };
  });
  const totalShareBase = byOperator.reduce((s, e) => s + e.sumXaf, 0) || 1;
  byOperator.forEach((e) => { e.marketSharePct = +(100 * e.sumXaf / totalShareBase).toFixed(2); });
  ...
  return {
    ...
    lifetime: { ...lifetime, byOperator: undefined },
    totals: {
      ...totals,
...
module.exports = { ingest, rebuildFrom, snapshot, selectRecent, recentTx, size, all, lifetime };
