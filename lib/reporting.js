...
// Plafond d'enregistrements par rapport. Correctif D1 : un rapport n'est plus une
// invitation à relire tout le registre dans le fil de requête — le parcours est
// déporté, borné, et la troncature éventuelle est REMONTÉE à l'appelant plutôt que
// silencieuse (un rapport réglementaire tronqué sans le dire serait pire qu'absent).
const REPORT_MAX_RECORDS = Number(process.env.SUMO_REPORT_MAX_RECORDS) || 200_000;
// Sans période explicite, on ne remonte pas à la genèse : fenêtre par défaut.
const REPORT_DEFAULT_DAYS = Number(process.env.SUMO_REPORT_DEFAULT_DAYS) || 366;

async function loadTdr({ start, end, operatorId }) {
  const to = end ? periodEnd(end) : Date.now();
  const from = start ? new Date(start).getTime() : to - REPORT_DEFAULT_DAYS * 86_400_000;
  const raw = await scanner.scan({
    kind: 'period', file: ledger.file, from, to, operatorId: operatorId || null, limit: REPORT_MAX_RECORDS,
  });
  const { records, scanStats } = scanner.materialize(raw, model.fromSealed);
  return { list: records.map((r) => r.payload), scanStats, window: { from, to } };
}

function kpisOf(list) {
  const sumXaf = list.reduce((s, t) => s + t.amountXaf, 0);
  const feeXaf = list.reduce((s, t) => s + (t.fee.amount || 0), 0);
  const taxXaf = list.reduce((s, t) => s + ((t.tax && t.tax.amount) || 0), 0);
  ...
  return {
    transactions: list.length, volumeXaf: sumXaf, fraisXaf: feeXaf, taxesXaf: taxXaf,
    // Le taux de succès porte sur les transactions dénouées ; PENDING et REVERSED
    // sont comptés à part pour ne fausser ni la QoS ni l'assiette.
    transactionsEnAttente: list.filter((t) => t.status === 'PENDING').length,
    transactionsAnnulees: list.filter((t) => t.status === 'REVERSED').length,
    tauxSucces: settled ? +(success / settled).toFixed(4) : 1,
  };
}
...
function redevances(list) {
  ...
  const rows = [...revenue.feeAggregates(list).entries()].map(([id, e]) => ({
    ...
    frais_declares: e.declared, frais_non_declares: e.undeclared,
    frais_percus_xaf: e.collected, frais_attendus_xaf: e.expected,
    taxes_percues_xaf: e.tax,
    ecart_xaf: e.expected - e.collected, sous_declarations: e.under, sur_facturations: e.over,
...
function qos(list) {
  ...
  const rows = [...byOp.entries()].map(([id, sub]) => {
    ...
    return {
      ...
      transactions_denouees: agg.settled,
      taux_succes_pct: +(100 * agg.successRate).toFixed(2),
      mesures_latence: agg.latencySample,
      latence_moy_ms: agg.avgLatencyMs == null ? 'NON MESUREE' : agg.avgLatencyMs,
      latence_p95_ms: agg.p95LatencyMs == null ? 'NON MESUREE' : agg.p95LatencyMs,
    };
  ...
  for (const t of list) {
    ...
    if (t.amountXaf >= thr) {
      overThreshold.push({
        ...
        type: t.type, montant_xaf: t.amountXaf, msisdn: ref.maskMsisdn(t.sender.msisdn), ville: t.cellOrigin ? t.cellOrigin.city : 'NON DECLAREE',
      });
    }
  }
...
async function generate(templateId, opts = {}) {
  const tpl = TEMPLATES.find((t) => t.id === templateId);
  if (!tpl) throw new Error(`Modèle de rapport inconnu: ${templateId}`);
  const { list, scanStats, window } = await loadTdr(opts);
  const body = BUILDERS[templateId](list);
  return {
    template: tpl.id, label: tpl.label,
    period: {
      start: opts.start || new Date(window.from).toISOString().slice(0, 10),
      end: opts.end || new Date(window.to).toISOString().slice(0, 10),
      operatorId: opts.operatorId || null,
      implicite: !opts.start && !opts.end,
    },
    generatedAt: new Date().toISOString(),
    recordCount: list.length,
    // Un rapport tronqué le DIT : il ne peut pas être opposé tel quel.
    truncated: scanStats.truncated,
    scanStats,
    ...body,
  };
}
...
module.exports = { TEMPLATES, generate, toCsv, periodEnd, loadTdr, REPORT_MAX_RECORDS };
