...
(function replay() {
  ...
  const slice = all.slice(-REPLAY_MAX);
  for (const r of slice) { pipeline.enrich(r.payload); warehouse.ingest(r.payload); }
  if (all.length) logger.info('replay.done', { replayed: slice.length, total: all.length });
}());
...
function broadcast(obj) {
  ...
  const rxId = isTx && obj.data && obj.data.receiverOperator ? obj.data.receiverOperator.id : null;
  for (const client of wss.clients) {
    if (client.readyState !== WebSocket.OPEN) continue;
    if (client.bufferedAmount > 1_000_000) continue;
    if (isTx && client.scope && client.scope.operatorId && opId !== client.scope.operatorId && rxId !== client.scope.operatorId) continue;
    client.send(msg);
  }
}
...
wss.on('connection', (ws, req) => {
  ...
  const inScope = (t) => !ws.scope.operatorId || t.operator.id === ws.scope.operatorId || t.receiverOperator.id === ws.scope.operatorId;
  ws.isAlive = true;
...
server.listen(PORT, HOST, () => {
  ...
  console.log(`  Comptes   : ${users.count()} intervenants (page : /login.html)`);
