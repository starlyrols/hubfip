...
// =============================================================================
// Application Express durcie — API REST de la plateforme SUMo (13 modules).
// Routes groupées par module ; RBAC par corps de métier (gating de module),
// cloisonnement opérateur (scope), minimisation des données (MSISDN masqués sauf
// rôle habilité) et journalisation d'audit des actions sensibles.
// =============================================================================
...
function createApp(opts = {}) {
  ...
  const publicUser = (s) => ({ username: s.username, displayName: s.displayName, role: s.role, operatorId: s.operatorId, scopeType: s.scopeType, title: s.title, permissions: users.permissions(s) });
  ...
  const requireModule = (moduleId) => (req, res, next) => {
    if (!req.auth) return res.status(401).json({ error: 'Authentification requise.' });
    if (!users.modulesFor(req.auth.role).includes(moduleId)) return res.status(403).json({ error: `Module « ${moduleId} » non autorisé pour votre profil.` });
    next();
  };
  const requireAdmin = (req, res, next) => {
    if (!req.auth) return res.status(401).json({ error: 'Authentification requise.' });
    if (!users.canAdmin(req.auth.role)) return res.status(403).json({ error: 'Action réservée à l\'administration.' });
    next();
  };
  ...
  const canReveal = (req) => !!req.auth && users.canReveal(req.auth.role);
  ...
  // ==========================================================================
  app.get('/api/v1/cases', requireModule('investigation'), (req, res) => res.json({ stats: cases.stats(), cases: cases.list({ status: req.query.status }) }));
  app.post('/api/v1/cases', requireModule('investigation'), (req, res) => {
    if (!users.canWriteCases(req.auth.role)) return res.status(403).json({ error: 'Création de dossier non autorisée pour votre profil.' });
    const b = req.body || {};
  ...
  app.get('/api/v1/cases/:id', requireModule('investigation'), (req, res) => {
    ...
    const out = { ...c, subjectMsisdn: canReveal(req) ? c.subjectMsisdn : ref.maskMsisdn(c.subjectMsisdn) };
    res.json({ case: out });
  });
  app.patch('/api/v1/cases/:id', requireModule('investigation'), (req, res) => {
    if (!users.canWriteCases(req.auth.role)) return res.status(403).json({ error: 'Modification non autorisée.' });
  ...
  app.get('/api/v1/ledger', requireModule('registre'), (req, res) => {
    ...
    const records = ledger.getRecent(800)
      .filter((r) => !o.operatorId || r.payload.operator.id === o.operatorId || r.payload.receiverOperator.id === o.operatorId)
      .slice(0, limit)
      .map((r) => ({ seq: r.seq, ts: r.ts, hash: r.hash, payload: model.toPublic(r.payload, { reveal: false }) }));
    res.json({ stats: ledger.stats(), records });
  });
  app.get('/api/v1/ledger/verify', requireModule('registre'), (_req, res) => res.json(ledger.verifyChain()));
  ...
  app.get('/api/v1/security', requireModule('securite'), async (_req, res) => {
    ...
    res.json({
      ...
      ledger: { ...ledger.stats(), integrity: ledger.verifyChain() },
      audit: { ...audit.stats(), integrity: audit.verify() },
      persistence, // entrepôt PostgreSQL (si DATABASE_URL défini)
      rbac: { roles: Object.keys(users.ROLES).length, modules: users.ALL_MODULES.length },
      dataMinimization: { msisdnMaskedByDefault: true, revealRoles: ['ANTIFRAUDE', 'JURIDIQUE', 'REGULATEUR'], revealAudited: true },
  ...
  app.post('/api/v1/iso8583', authenticate, (req, res) => {
    try {
      ...
      if (typeof p.message === 'string') {
        tdr = model.fromIso8583(p.message, { operatorId: p.operatorId, channel: p.channel, senderMsisdn: p.senderMsisdn });
