...
// Plafonné en nombre de clés (éviction LRU) — cardinalité illimitée possible via
// l'injection externe, et chaque clé retient jusqu'à HIST_MAX entrées.
const history = new Map();
const HIST_MAX = 60;
const HIST_KEYS_MAX = 50_000;
...
function pushHistory(msisdn, epoch, amountXaf) {
  ...
  if (h) history.delete(msisdn); // réinsertion → l'ordre du Map reflète la récence
  else {
    h = [];
    if (history.size >= HIST_KEYS_MAX) history.delete(history.keys().next().value);
  }
  history.set(msisdn, h);
...
function evaluate(tdr) {
  ...
  for (const a of alerts) {
    ...
    recentAlerts.push({
      ...
      city: tdr.cellOrigin ? tdr.cellOrigin.city : null, ruleId: a.ruleId, severity: a.severity, label: a.label, detail: a.detail,
    });
