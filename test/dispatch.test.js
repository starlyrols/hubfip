...
test('GET /api/v1/dispatch/state : 401 sans session, 403 hors admin système, 200 pour admin-systeme', async () => {
  ...
  assert.equal(j.modules[0].children.length, 13);
...
test('Dispatcher « monitoring » (parent) = les 13 sous-modules, y compris admin/config', async () => {
  const admin = await authCookie('admin-systeme');
  ...
  assert.equal(me.user.permissions.modules.length, 13);
