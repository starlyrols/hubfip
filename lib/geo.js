...
// Nombre de TDR écartés faute de localisation déclarée — la couverture du module
// doit être visible, sinon une carte « vide » se confond avec une absence d'activité.
...
function evaluate(tdr) {
  ...
  // Correctif A2 : sans cellule DÉCLARÉE, aucune corrélation géographique n'est
  // possible. On ne rabat plus sur une cellule par défaut — ce qui produisait une
  // cartographie biaisée et de faux « déplacements impossibles ».
  if (!cell) { sansLocalisation++; return null; }
  let alert = null;
...
function stats() {
  ...
  return {
    totalCells: ref.CELLS.length, activeCells: active, anomalies: recentGeo.length,
    trackedMsisdn: lastSeen.size, sansLocalisation,
  };
}
